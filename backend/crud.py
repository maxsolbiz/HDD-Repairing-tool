...
from models import User, UserActivity, UserRoleEnum
...
def create_user(db: Session, email: str, password: str, username: str = None, role: str = "user"):
    try:
        ...
        # Convert the role to the enum type; this will raise an error if the role is invalid
        role_enum = UserRoleEnum(role)
        db_user = User(email=email, hashed_password=hashed_pw, username=username, role=role_enum)
        db.add(db_user)
        db.commit()
        db.refresh(db_user)
        return db_user
